...
st.set_page_config(page_title="Enterprise Command Centre", layout="wide", page_icon="🏢", initial_sidebar_state="collapsed")
...
# ============================================
def check_trial(cid):
    """Check if company trial is still active. Returns (is_active, days_left, trial_end)."""
    c = get_db()
...
def start_trial(cid):
    """Start 90-day trial for new company."""
...
def ai_chat(question):
    """AI that ALWAYS responds — with fallback if API is down."""
    try:
        r = requests.post(
            "https://api.openai.com/v1/chat/completions",
            headers={"Authorization": f"Bearer {OPENAI_API_KEY}", "Content-Type": "application/json"},
            json={"model": "gpt-4o-mini", "messages": [{"role": "system", "content": "Enterprise assistant. Be concise and helpful."}, {"role": "user", "content": question}], "max_tokens": 300},
            timeout=20
        )
    ...
    if "fleet" in q or "vehicle" in q:
        return "📊 **Fleet Analysis:**\n\n• Daily walkaround checks are legally required before vehicle use\n• Common issues: tyre wear, brake efficiency, lighting faults\n• Preventive maintenance every 6-8 weeks recommended\n• Regular inspections reduce major repairs by 40%\n\n_Connect OpenAI API key for detailed AI analysis_"
    elif "safety" in q or "risk" in q:
        return "⚠️ **Safety Analysis:**\n\n• Encourage all near-miss reporting — most accidents have warning signs\n• Refresher training every 12 months reduces incidents by 35%\n• Inspect PPE monthly\n• Lone workers: implement 4-hour check-in policy\n\n_Connect OpenAI API key for detailed AI analysis_"
    elif "cost" in q or "save" in q or "reduce" in q:
        return "💡 **Cost Reduction:**\n\n• Monitor MPG per vehicle — target 10% improvement\n• Preventive maintenance is 3x cheaper than reactive repairs\n• Eco-driving training saves 15% on fuel\n• Digital records eliminate paper waste\n\n_Connect OpenAI API key for detailed AI analysis_"
    elif "compliance" in q or "dvsa" in q or "hse" in q:
        return "📋 **Compliance Overview:**\n\n• DVSA: Daily walkaround checks mandatory for HGVs\n• HSE: Record all near misses and investigate within 24 hours\n• Keep inspection records for minimum 15 months\n• Operator licence requires continuous compliance monitoring\n\n_Connect OpenAI API key for detailed AI analysis_"
    else:
        return f"🤖 **Enterprise Assistant**\n\nI can help with:\n• 📊 Fleet management & vehicle inspections\n• ⚠️ Safety compliance & near-miss reporting\n• 💡 Cost reduction strategies\n• 👥 Staff management\n• 📋 DVSA/HSE regulatory compliance\n\n_Connect OpenAI API key for detailed AI responses_"
...
if "logged_in" not in st.session_state:
    ...
    st.session_state.dr = False
    st.session_state.st = None
    st.session_state.tmpl = ""
...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        <div style="text-align:center;margin-bottom:50px;">
            ...
            <h1 style="font-size:2.8em;font-weight:800;letter-spacing:-2px;color:#f1f1f3;margin:0;">Enterprise</h1>
            <p style="color:#8888a0;font-size:1.1em;">Command Centre</p>
            <p style="color:#f59e0b;font-size:0.9em;">3-Month Free Trial — No Card Required</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with tab1:
            with st.form("login"):
                ...
                if st.form_submit_button("Sign In", type="primary", use_container_width=True):
                    c = get_db()
                    r = c.execute("SELECT password, role, company_id FROM users WHERE username=?", (u,)).fetchone()
                    c.close()
                    if r and Security.verify(p, r[0]):
                        # Check trial
                        active, days, end = check_trial(r[2])
                        if not active:
                            st.error("⚠️ Your free trial has expired. Please contact us to upgrade.")
                        else:
                            st.session_state.logged_in = True
                            st.session_state.user = u
                            st.session_state.role = r[1]
                            st.session_state.cid = r[2]
                            st.rerun()
                    else:
        ...
        with tab2:
            with st.form("register"):
                ...
                <b>🕐 3-Month Free Trial</b><br>
                <small>Full access to all features. No credit card required. Upgrade anytime.</small>
                </div>
                """, unsafe_allow_html=True)
                
                if st.form_submit_button("Start Free Trial", type="primary", use_container_width=True):
                    if not co or not au or not ap:
                        st.error("All fields required")
                    elif len(ap) < 8:
                        st.error("Password: 8+ characters")
                    else:
                        try:
                            c = get_db()
                            c.execute("INSERT INTO companies (name) VALUES (?)", (co,))
                            cid = c.execute("SELECT last_insert_rowid()").fetchone()[0]
                            c.execute("INSERT INTO users (username, password, role, company_id) VALUES (?, ?, 'admin', ?)", (au, Security.hash(ap), cid))
                            c.commit()
                            c.close()
                            start_trial(cid)
                            st.success("✅ Trial started! You have 90 days of full access. Go to Sign In.")
                            st.balloons()
                        except:
                            st.error("Company or username already exists")
    st.stop()
...
if not trial_active:
    ...
    <div class="trial-expired">
        ...
        <p style="color:#8888a0;font-size:1.2em;">Your 3-month free trial has ended.</p>
        <p style="color:#f1f1f3;">To continue using Enterprise Command Centre, please upgrade your account.</p>
        <br>
        <p style="color:#8888a0;">Contact: <b>davidmlangstead@gmail.com</b></p>
    </div>
    """, unsafe_allow_html=True)
...
# ============================================
with st.sidebar:
    st.markdown(f'<div style="padding:10px 0;"><div style="font-size:1.3em;font-weight:800;color:#f1f1f3;">◆ Enterprise</div></div>', unsafe_allow_html=True)
    ...
    if days_left <= 30:
        ...
        st.markdown(f'<div style="background:rgba({ "239,68,68" if days_left <= 7 else "245,158,11" },0.1);border:1px solid {color};border-radius:10px;padding:10px;margin-bottom:12px;text-align:center;"><b style="color:{color};">{days_left} days left</b><br><small style="color:#8888a0;">of free trial</small></div>', unsafe_allow_html=True)
...
if page == "Dashboard":
    ...
    # Trial warning
    if days_left <= 30:
        st.markdown(f'<div class="trial-banner">⚠️ <b>{days_left} days remaining</b> in your free trial. <a href="#" style="color:#4f6ef7;">Upgrade now</a> to avoid interruption.</div>', unsafe_allow_html=True)
...
# ============================================
    ...
    with tab1:
        ...
        else: st.info("No vehicles registered")
    with tab2:
        with st.form("add_v"):
...
# ============================================
    ...
    with c1:
        h = st.session_state.dt // 3600; m = (st.session_state.dt % 3600) // 60
        st.markdown(f'<div style="background:#16161f;border:1px solid #222233;border-radius:16px;padding:30px;text-align:center;"><div style="color:#8888a0;">DRIVING</div><div style="font-size:3em;font-weight:800;color:#22c55e;">{h:02d}:{m:02d}</div></div>', unsafe_allow_html=True)
    with c2:
        rem = max(0, 16200 - st.session_state.dt); rh = rem // 3600; rm = (rem % 3600) // 60
        st.markdown(f'<div style="background:#16161f;border:1px solid #222233;border-radius:16px;padding:30px;text-align:center;"><div style="color:#8888a0;">REMAINING</div><div style="font-size:3em;font-weight:800;color:#f59e0b;">{rh:02d}:{rm:02d}</div></div>', unsafe_allow_html=True)
    with c3:
...
# ============================================
    ...
    st.markdown('<h1>AI Assistant</h1><p style="color:#8888a0;">Always available — smart responses even when offline</p>', unsafe_allow_html=True)
    q = st.text_area("Your Question", value=st.session_state.ai_q, height=100, placeholder="Ask anything about fleet, safety, compliance, or business...")
...
# ============================================
    ...
    <div class="glass-card">
        <h3>Trial Status</h3>
        <p><b>Days Remaining:</b> <span style="color:{'#ef4444' if days_left <= 7 else '#f59e0b' if days_left <= 30 else '#22c55e'};">{days_left} days</span></p>
        <p><b>Trial End Date:</b> {trial_end}</p>
        <p style="color:#8888a0;">To upgrade, contact: davidmlangstead@gmail.com</p>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("---")
st.markdown(f'<div style="text-align:center;color:#8888a0;padding:20px;">◆ Enterprise Command Centre • {days_left} days remaining in trial</div>', unsafe_allow_html=True)
