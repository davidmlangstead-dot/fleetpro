...
conn.execute("CREATE TABLE IF NOT EXISTS training (id INTEGER PRIMARY KEY AUTOINCREMENT, company_id INTEGER, title TEXT, description TEXT, video_url TEXT, required_roles TEXT)")
...
def ai_analyze(prompt):
    ...
    try:
        r=requests.post("https://api.openai.com/v1/chat/completions",headers={"Authorization":f"Bearer {OPENAI_API_KEY}","Content-Type":"application/json"},json={"model":"gpt-4o-mini","messages":[{"role":"system","content":"Expert. 3 bullet points max."},{"role":"user","content":prompt}],"max_tokens":200},timeout=10)
...
if page=="📖 Help Guide":
    ...
    with st.expander("🚀 GETTING STARTED",expanded=True):
        st.markdown("""<div class="instruction-step"><h3>1️⃣ Register Your Company</h3><p>Click <b>Register</b> on the login page. Enter company name, admin username and password (8+ characters).</p></div>
        <div class="instruction-step"><h3>2️⃣ Add Your Sites</h3><p>Go to <b>📍 Sites</b>. Add each location — depot, warehouse, office.</p></div>
        <div class="instruction-step"><h3>3️⃣ Add Your Team</h3><p>Go to <b>👥 Staff</b>. Create accounts for each employee. Roles: Admin, Manager, Driver, Workshop, Worker.</p></div>
    ...
    with st.form("upload_doc"):
        ...
        if st.form_submit_button("Upload",type="primary",use_container_width=True):
            if filename and file:content=base64.b64encode(file.read()).decode()[:1000];uid=sq("SELECT id FROM users WHERE username=?",(st.session_state.user,)).iloc[0,0];db.execute("INSERT INTO documents (company_id,filename,description,uploaded_by,file_data) VALUES (?,?,?,?,?)",(cid,filename,desc,uid,content));st.success("✅ Uploaded!");st.rerun()
    ...
    with ca:
        with st.form("add_v"):
            reg=st.text_input("Reg*").upper();t=st.selectbox("Type*",["HGV Artic","HGV Rigid","Van","Car"]);make=st.text_input("Make");model=st.text_input("Model")
    ...
    with st.form("add_site"):
        sn=st.text_input("Site Name*");sa=st.text_input("Address")
        if st.form_submit_button("Add",type="primary",use_container_width=True):
    ...
    with st.form("repair"):
        ...
        if st.form_submit_button("Mark Repaired",type="primary",use_container_width=True):c=get_db();c.execute("INSERT INTO ops (time,reg,mileage,status,notes,driver,company_id) VALUES (?,?,0,'REPAIRED',?,?,?)",(datetime.now(),reg,rn,st.session_state.user,cid));c.commit();c.close();st.success("✅ Repaired!");st.rerun()
    ...
    with st.form("fuel"):
        vehs=sq("SELECT reg FROM vehicles WHERE company_id=?",(cid,));reg=st.selectbox("Vehicle",vehs['reg'].tolist() if not vehs.empty else ["None"]);lit=st.number_input("Litres",0.0,1000.0,0.0);cost=st.number_input("Cost (£)",0.0,10000.0,0.0);mil=st.number_input("Mileage",0,999999,0)
    ...
    with st.form("maint"):
        vehs=sq("SELECT reg FROM vehicles WHERE company_id=?",(cid,));reg=st.selectbox("Vehicle",vehs['reg'].tolist() if not vehs.empty else ["None"]);stp=st.selectbox("Type",["Oil Change","Brake Service","MOT","Annual Service"]);dd=st.date_input("Due Date")
    ...
    with st.form("add_training"):
        title=st.text_input("Course Title*");desc=st.text_area("Description")
        if st.form_submit_button("Add",type="primary",use_container_width=True):
    ...
    with st.form("new_form"):
        fn=st.text_input("Form Name*");fields=st.text_area("Fields (one per line)")
        if st.form_submit_button("Create",type="primary",use_container_width=True):
    ...
    st.button("🚨 EMERGENCY",type="secondary",use_container_width=True)
